  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4">
        <span className="text-gray-600 text-sm">{t('outbound.selectedOrders')} {selectedOrders.length}/{availableOrders.length} {t('outbound.orders')}</span>
        <div className="flex gap-2 w-full sm:w-auto">
          <button
            onClick={onAICreatePickOrders}
            disabled={aiLoading || availableOrders.length === 0 || !canWrite}
            title={!canWrite ? t('outbound.noPermission') : ''}
          ...
          >
            ...
            <span className="sm:hidden">{t('outbound.aiCreatePickOrders')}</span>
            <span className="hidden sm:inline">{t('outbound.aiCreatePickOrders')}</span>
          </button>
          {canWrite && (
            <button
            ...
            >
              <span className="sm:hidden">{t('outbound.createPickOrder')}</span>
              <span className="hidden sm:inline">{t('outbound.createPickOrder')}</span>
            </button>
          )}
        </div>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="text-center text-gray-500 text-sm border-b">
              ...
              <th className="pb-3 text-center">{t('outbound.orderNo')}</th>
              <th className="pb-3 text-center">{t('outbound.createTime')}</th>
              <th className="pb-3 text-center">{t('outbound.entity')}</th>
              <th className="pb-3 text-center">{t('outbound.warehouse')}</th>
              <th className="pb-3 text-center">{t('outbound.products')}</th>
              <th className="pb-3 text-center">{t('outbound.actions')}</th>
            </tr>
          </thead>
          <tbody>
                        ...
                        <div key={`${item.id}-${idx}`} className="text-sm mb-1 flex items-center justify-center">
                          ...
                          {((lock as any).skuBatch?.batchNo || (lock as any).bundleBatch?.batchNo) && (
                            <span className="ml-1 text-purple-500 text-xs">
                              {t('inventory.batch')}:{(lock as any).skuBatch?.batchNo || (lock as any).bundleBatch?.batchNo}
                            </span>
                          )}
                        </div>
                      );
          ...
          >
            ...
            <div className="text-xs text-gray-600 space-y-1 mb-2">
              <div className="flex justify-between">
                ...
                <span>{order.items?.length || 0}{t('outbound.products')}</span>
              </div>
            </div>
            <div className="space-y-1.5 border-t pt-2">
                  ...
                  <div key={item.id} className="text-xs bg-gray-50 rounded p-1.5">
                    ...
                    <div className="flex items-center justify-between text-gray-400 mt-0.5">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {batchNo && <span className="text-purple-500">{t('inventory.batch')}:{batchNo}</span>}
                      </div>
                    </div>
                  </div>
                );
              ...
              {(order.items?.length || 0) > 3 && (
                <div className="text-xs text-gray-400 text-center">{t('common.total')} {(order.items?.length || 0) - 3} {t('outbound.products')}...</div>
              )}
            </div>
          </div>
        ))}
        {availableOrders.length === 0 && (
          <div className="text-center py-8 text-gray-500">{t('outbound.noPendingOrders')}</div>
        )}
      </div>
    </div>
  );
