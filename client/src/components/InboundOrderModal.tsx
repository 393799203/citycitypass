...
export default function InboundOrderModal({
  ...
  const handleCreateOrder = async () => {
    ...
    if (inboundItems.some(item => item.quantity > 0 && !item.locationId)) {
      toast.warning('请为所有入库商品选择库位');
      return;
    }
        ...
        <div className="flex flex-1 overflow-hidden">
          {isReturn || inboundItems.length > 0 ? (
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="flex gap-2 p-3 sm:p-4 border-t bg-gray-50">
                <button onClick={onClose} className="flex-1 px-3 sm:px-4 py-2 border rounded-lg text-sm">取消</button>
                <button
                  onClick={handleCreateOrder}
                  disabled={saving || inboundItems.every(item => item.quantity === 0) || inboundItems.some(item => item.quantity > 0 && !item.locationId)}
                  className="flex-1 px-3 sm:px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 text-sm"
                >
                  {saving ? '创建中...' : '创建入库单'}
                </button>
              </div>
            ...
            <div className="flex-1 flex flex-col gap-2">
              ...
              <div className="flex gap-2 p-3 sm:p-4 border-t bg-gray-50">
                <button onClick={onClose} className="flex-1 px-3 sm:px-4 py-2 border rounded-lg text-sm">取消</button>
                <button disabled className="flex-1 px-3 sm:px-4 py-2 bg-gray-300 text-gray-500 rounded-lg text-sm">创建入库单</button>
              </div>
            </div>
          )}
        </div>
