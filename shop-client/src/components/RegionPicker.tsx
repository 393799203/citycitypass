      ...
      {open && (
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border rounded-lg shadow-lg max-h-64 overflow-hidden">
          <div className="flex border-b">
            {showRegion && (
              <button
                className={`flex-1 px-3 py-2 text-sm ${step === 'region' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-500'}`}
                onClick={handleBack}
              >
            ...
            <button
              className={`flex-1 px-3 py-2 text-sm ${step === 'province' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-500'}`}
            ...
            <button
              className={`flex-1 px-3 py-2 text-sm ${step === 'city' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-500'}`}
          ...
          <div className="overflow-y-auto max-h-48">
            {step === 'region' && showRegion && regions.map(r => (
              <div
                key={r.value}
                className={`px-4 py-2 cursor-pointer hover:bg-gray-50 ${regionValue === r.value ? 'bg-blue-50 text-blue-600' : ''}`}
            ...
            {step === 'province' && getProvinces().map(p => (
              <div
                key={p.value}
                className={`px-4 py-2 cursor-pointer hover:bg-gray-50 ${value.province === p.value ? 'bg-blue-50 text-blue-600' : ''}`}
            ...
            {step === 'city' && (() => {
              ...
              return provinces ? provinces.cities.map(c => (
                <div
                  key={c}
                  className={`px-4 py-2 cursor-pointer hover:bg-gray-50 ${value.city === c ? 'bg-blue-50 text-blue-600' : ''}`}
